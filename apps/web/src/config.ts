...
export const CONFIG = {
  ...
  /**
   * PLATEAU 3D Tiles manifest, generated by scripts/data/build-plateau-manifest.mjs
   * from MLIT's official GraphQL catalog. The URLs inside use the `latest` composite
   * endpoint, so no data year is hardcoded (spec §9) without putting a third-party API
   * on the critical path of every page load.
   */
  plateauManifestUrl: str("VITE_PLATEAU_MANIFEST_URL", "data/plateau-manifest.json"),
